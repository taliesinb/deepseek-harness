...
Every Session row's menu offers **Move to…** and every Workspace row's **Rehome workspace…**; dragging a Session row onto another Workspace group also moves it. Both dialogs pick the destination with the same flow as "Add workspace" (an existing Workspace, or a directory registered on the way) and call the Session Controller's `move` / `moveMany`. A Session with nothing in flight moves immediately (its resident Agent, if any, is retired and resumes cold in the new Workspace). The Host refuses when retiring would interrupt work — a running turn, background jobs, loaded subagents — and the move dialog lists exactly those blockers before offering "Interrupt this work and move it". Rehome moves every member of the source Workspace, reports skips, and can delete the emptied source. Each moved Agent receives a notice on its next step unless "Tell the agent" is unticked. **Copy to…** on the same menu is the non-destructive twin: it calls the Session Controller's `copy`, which stores a new Session (fresh id, subagent descendants along) under the chosen Workspace — the Session's own workspace is preselected, so the item doubles as Duplicate — and never touches the source; the dialog prefills the copy's title as “<title> (copy)” (editable; an unchanged title keeps the source's), and lands on the copy when done. A running source is refused once (`session/copy-live`); the dialog then lists what is in flight and offers **Copy only up to the last completed turn** (ticked by default: the turn in progress and its prompt are left out; unticked: everything recorded so far is kept and that turn is closed as interrupted) before the operator confirms again. Both session dialogs also list destinations other plugins contribute through `ctx.uiWorkspace.contributeDestinations({ id, order, groups, run })` — `groups` is an observable list of `{ id, label, entries: [{ key, title, path? }] }` (one remote host each, say) — as further headed groups after **This machine** in a tree-shaped picker; picking one hands the whole operation to the contributor's `run({ action, sessionId, sourceWorkspaceId?, sourceTitle, groupId, destinationKey, stopLive?, truncate?, title?, notify })`, which answers `{ ok: true, summary? }` or `{ ok: false, code, message, details? }` using the Host's own refusal codes (`session/move-live`, `session/copy-live`), so the stop-and-move / truncate flows read the same for every destination. A `summary` is shown before the dialog closes; without contributions the picker is the flat local list.
