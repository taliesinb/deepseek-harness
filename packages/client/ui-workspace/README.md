...
Every Session row's menu offers **Move to…** and every Workspace row's **Rehome workspace…**; dragging a Session row onto another Workspace group also moves it. Both dialogs pick the destination with the same flow as "Add workspace" (an existing Workspace, or a directory registered on the way) and call the Session Controller's `move` / `moveMany`. An idle Session moves immediately (its resident Agent, if any, is retired and resumes cold in the new Workspace). Only a Session whose Agent is mid-turn is refused by the Host; the move dialog answers by revealing "Interrupt the current turn and move it" so the operator decides. Rehome moves every member of the source Workspace, reports skips, and can delete the emptied source. Each moved Agent receives a notice on its next step unless "Tell the agent" is unticked.
