...
One optional sixth method, `relocate({ id, cwd, append? })`, moves a **cold** stored session to another working directory: the header's `cwd` is rewritten, the given events are stamped after the stored tail as part of the same durable rewrite, the artifact is republished where the new cwd places it, and the old one is retired (the JSONL backend keeps it under `<root>/../session-move-backups/`). A session open for writing anywhere refuses with `SessionAlreadyOwnedError`. Backends that cannot relocate leave the method undefined; the Session Controller's `move` reports `session/move-unsupported`.
