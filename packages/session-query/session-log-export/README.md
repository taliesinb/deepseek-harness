...
The same plugin mounts the receiving half of a cross-host move: `POST /api/session.import?workspaceId=<id>` (or `?cwd=<dir>` to register the directory as a Workspace on the way) with an export ZIP as the body. Its logs are stored as Sessions of that Workspace with the destination cwd; `media/` and `files/` entries are saved through the attachment store, whose content addressing keeps the logs' references valid; an exported id already present here is replaced by a fresh one (subagent children re-parented to match; pass `keepIds=false` to always re-id); the root Session receives one `agent/inbox/spliced` next-step notice naming the origin (`origin=` query, default "another DSH host") so the Agent knows earlier file references may not resolve (`notify=false` omits it). The response is `{ sessionId, imported: [{ sessionId, exportedId, parentSessionId? }], attachments }`; malformed archives answer 422, an unknown Workspace 404, archives above 256 MiB 413. With `mode=copy` the same route is the receiving half of a cross-host *copy*: every log gets a fresh id regardless of collisions, a source log with a turn in progress is shaped by `shapeCopiedLog` — `truncate=true` cuts it just before the open turn's `turn/start` and cancels the prompt still queued for that turn (descendants created after the cut are left out), otherwise the open turn is closed with `interruptedTurnClosers` — an optional `title=` is recorded as a `session/title` event, and the notice says the Session is a copy whose original continues elsewhere. The response then also carries `truncated`. The same storing half (`storeSessionLogs`) and `copyStoredSession` (read a stored Session and its lineage through persistence, flushing live ones) serve the Session Controller's same-host `session.copy`. The route needs `sessionPersistence`, `attachments`, and `workspaceRegistry` to be mounted.
